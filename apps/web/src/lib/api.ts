...
class ApiClient {
  ...
  properties = {
    ...
    delete: (id: string) => this.request<void>(`/api/v1/properties/${id}`, { method: 'DELETE' }),
  }
  ...
  jobs = {
    ...
    accept: (id: string) => this.request<any>(`/api/v1/jobs/${id}/accept`, { method: 'POST' }),

    decline: (id: string) => this.request<any>(`/api/v1/jobs/${id}/decline`, { method: 'POST' }),

    start: (id: string) => this.request<any>(`/api/v1/jobs/${id}/start`, { method: 'POST' }),

    complete: (id: string) => this.request<any>(`/api/v1/jobs/${id}/complete`, { method: 'POST' }),

    cancel: (id: string) => this.request<any>(`/api/v1/jobs/${id}/cancel`, { method: 'POST' }),
  }

  // Reviews endpoints
  reviews = {
    list: () => this.request<any>('/api/v1/reviews/'),
  ...
  messages = {
    ...
    getConversation: (id: string) => this.request<any>(`/api/v1/messages/conversations/${id}`),
  ...
  notifications = {
    ...
    markAllRead: () => this.request<void>('/api/v1/notifications/read-all', { method: 'POST' }),
  }

  // Feed endpoints
  feed = {
    list: (page = 1, limit = 20) => this.request<any>(`/api/v1/feed?page=${page}&limit=${limit}`),

    like: (id: string) => this.request<any>(`/api/v1/feed/${id}/like`, { method: 'POST' }),
    ...
    delete: (id: string) => this.request<void>(`/api/v1/feed/${id}`, { method: 'DELETE' }),
  }
  ...
  admin = {
    ...
    users: (page = 1, role?: string, status?: string, q?: string) => {
    ...
    // --- analytics time-series (real trends) ---
    timeseries: (range = '30d') =>
      this.request<any>(`/api/v1/admin/analytics/timeseries?range=${range}`),

    // --- support toolkit ---
    forceLogout: (id: string) =>
      this.request<any>(`/api/v1/admin/users/${id}/force-logout`, { method: 'POST' }),

    passwordResetLink: (id: string) =>
      this.request<{ reset_link: string }>(`/api/v1/admin/users/${id}/password-reset-link`, {
        method: 'POST',
      }),

    impersonate: (id: string) =>
      this.request<{ access_token: string; user: any }>(`/api/v1/admin/users/${id}/impersonate`, {
        method: 'POST',
      }),

    // CSV download — needs blob handling, so it bypasses request() (which JSON-parses).
    exportUsers: async () => {
      const res = await fetch(`${this.baseUrl}/api/v1/admin/export/users`, {
        headers: this.token ? { Authorization: `Bearer ${this.token}` } : {},
      })
      if (!res.ok) throw { detail: 'Export failed', status: res.status } as ApiError
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'bookacleaner-users.csv'
    ...
    // --- admin MFA ---
    mfaStatus: () => this.request<{ mfa_enabled: boolean }>('/api/v1/admin/mfa/status'),
    mfaSetup: () =>
      this.request<{ secret: string; otpauth_uri: string }>('/api/v1/admin/mfa/setup', {
        method: 'POST',
      }),
    mfaEnable: (code: string) =>
      this.request<any>('/api/v1/admin/mfa/enable', {
        method: 'POST',
        body: JSON.stringify({ code }),
      }),
    mfaDisable: (code: string) =>
      this.request<any>('/api/v1/admin/mfa/disable', {
        method: 'POST',
        body: JSON.stringify({ code }),
      }),
  ...
  bids = {
    ...
    accept: (id: string) => this.request<any>(`/api/v1/bids/bids/${id}/accept`, { method: 'POST' }),

    decline: (id: string) =>
      this.request<any>(`/api/v1/bids/bids/${id}/decline`, { method: 'POST' }),

    withdraw: (id: string) => this.request<any>(`/api/v1/bids/bids/${id}`, { method: 'DELETE' }),
  }
  ...
  settings = {
    ...
    deleteAccount: () =>
      this.request<void>('/api/v1/privacy/delete', {
        method: 'POST',
