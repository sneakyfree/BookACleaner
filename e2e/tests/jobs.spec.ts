...
test.describe('Job Lifecycle', () => {
  test.describe('Cleaner Job Management', () => {
    ...
    test('should display cleaner dashboard with jobs', async ({ page }) => {
      ...
      await expect(page.getByText(/job|booking|schedule|calendar|dashboard/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
    ...
    test('should show job details', async ({ page }) => {
      ...
      await expect(page.getByText(/job|booking|pending|no jobs|schedule/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
    ...
    test('should display calendar view', async ({ page }) => {
      ...
      await expect(page.getByText(/calendar|schedule|today|week|month/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
    ...
    test('should show earnings dashboard', async ({ page }) => {
      ...
      await expect(page.getByText(/earning|payment|balance|payout|\$/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
    ...
    test('should display verification status', async ({ page }) => {
      ...
      await expect(page.getByText(/verification|tier|verified|document/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
  })
  ...
  test.describe('Job Actions', () => {
    ...
    test('should have accept/decline buttons for pending jobs', async ({ page }) => {
      ...
      await expect(page.getByText(/job|booking|pending|schedule/i).first()).toBeVisible({
        timeout: 5000,
      })
    })
  ...
  test.describe('Messaging', () => {
    ...
    test('should display messages page', async ({ page }) => {
      ...
      await expect(
        page.getByText(/message|conversation|chat|inbox|no messages/i).first()
...
test.describe('Admin Job Management', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.fill('input[name="password"]', 'demo1234')
    await page.click('button[type="submit"]')
    // Wait for the session redirect so subsequent gotos are authenticated
  ...
  test('should display admin dashboard', async ({ page }) => {
    ...
    await expect(page.getByText(/user|client|cleaner|role|manage|admin/i).first()).toBeVisible({
      timeout: 5000,
    })
  })
  ...
  test('should display admin users list', async ({ page }) => {
    ...
    await expect(page.getByText(/user|client|cleaner|role|manage/i).first()).toBeVisible({
      timeout: 5000,
    })
  })
  ...
  test('should display verification queue', async ({ page }) => {
    ...
    await expect(page.getByText(/verification|pending|review|approve|queue/i).first()).toBeVisible({
      timeout: 5000,
    })
  })
})
